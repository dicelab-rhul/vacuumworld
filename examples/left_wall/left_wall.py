...
class MyMind(VWActorMindSurrogate):
    ...
    def revise(self, observation: VWObservation, messages: Iterable[BccMessage]) -> None:
        ...
        # We are done.
        if not self.__facing_left_wall and self.__get_own_appearance().is_facing_west() and self.__observation.is_wall_immediately_ahead():
            print("I am at the left wall - success!")

            self.__facing_left_wall = True
        # One more step.
        elif not self.__facing_left_wall and self.__get_own_appearance().is_facing_west() and self.__observation.is_wall_one_step_ahead():
    ...
    def __get_own_appearance(self) -> VWActorAppearance:
        ...
        '''
        return self.__observation.get_center().get_actor_appearance()
