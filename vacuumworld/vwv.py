...
class VWInterface(tk.Frame):
    ...
    def _size_slider_option(self, parent, bg):
        ...
        t.pack(side='left')
        increments = Environment.GRID_MAX_SIZE - Environment.GRID_MIN_SIZE
        self.grid_scale_slider = Slider(f, self.on_resize, None, increments * 16, 16, 
                                        increments=increments,
                                        start=(DEFAULT_GRID_SIZE/DEFAULT_LOCATION_SIZE) - Environment.GRID_MIN_SIZE)
    ...
    def _construct_images(self, img, name):
        return odict({name + 'north':img, name + 'east':img.copy().rotate(90), name + 'south':img.copy().rotate(180), name + 'west':img.copy().rotate(270)})
    ...
    def on_resize(self, value):
        value =  value + Environment.GRID_MIN_SIZE
...
def run():  
    ...
    try:
        ...
        global env
        env = Environment(INITIAL_ENVIRONMENT_DIM)
