...
class Observation(Perception):
    ...
    def merge_action_result_with_previous_observations(self, observations: Iterable[Observation]) -> None:
        '''
        WARNING: This method needs to be public, but it is not part of the public API of the `Observation` class.

        Merges this `Observation` with another `Observation` passed as an argument.

        There is not change to any `VWLocation` in this `Observation`.

        Each `ActionResult` in the `Observation` passed as an argument is prepended to the list of `ActionResult` in this `Observation`, as this `Observation` is the most recent one.
        '''
    ...
    def is_empty(self) -> bool:
        '''
        Returns whether or not this `Observation` is empty, i.e. whether or not it contains any `VWLocation`.
        '''
        return not self.__locations

    def get_locations(self) -> Dict[PositionNames, VWLocation]:
        '''
        Returns a `Dict` mapping each `PositionNames` to the `VWLocation` at that position.
    ...
    def get_location_at(self, position_name: PositionNames) -> Optional[VWLocation]:
        '''
        Returns the `VWLocation` at the given `PositionNames`, or `None` if there is no `VWLocation` at that position.
        '''
    ...
    def is_wall_one_step_ahead(self) -> bool:
        ...
        One step in front means that the `VWLocation` in front of the `VWActor` exists, and has a wall on its `forward` side (w.r.t the `Orientation` of the `VWActor`).
        '''
    ...
    def is_wall_one_step_to_the_left(self) -> bool:
        ...
        One step to the `left` means that the `VWLocation` to the `left` of the `VWActor` exists, and has a wall on its `left` side (w.r.t the `Orientation` of the `VWActor`).
        '''
    ...
    def is_wall_one_step_to_the_right(self) -> bool:
        ...
        One step to the `right` means that the `VWLocation` to the `right` of the `VWActor` exists, and has a wall on its `right` side (w.r.t the `Orientation` of the `VWActor`).
        '''
