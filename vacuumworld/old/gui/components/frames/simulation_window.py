...
class VWSimulationWindow(Frame):
    ...
    def __rotate_agent(self, _, direction: Direction) -> None:
        if self.__selected and self.__selected.actor:
            print(self.__selected)
            self.__remove_agent(self.__selected.coordinate)
            new_orientation: Direction =  direction(self.__selected.actor.orientation).value
            inc: int = self.__config["grid_size"] / self.__grid.dim
            tk_img: ImageTk.PhotoImage = self.__all_images_tk_scaled[(self.__selected.actor.colour.value, new_orientation)]
    ...
    def redraw(self) -> None:
        ...
        for coord, location in self.__grid.state.items():
            if location:
                if location.actor:
                    tk_img: ImageTk.PhotoImage = self.__all_images_tk_scaled[(location.actor.colour.value, location.actor.orientation.value)]
    ...
    def __drag_on_drop(self, event: Event, drag_manager: CanvasDragManager) -> None:
        ...
        if obj == "dirt":
            dirt1: DirtInterface =  self.__grid.dirt(colour)
            ...
            assert obj == "north"
            agent1: Actor =  self.__grid.agent(colour, obj)
