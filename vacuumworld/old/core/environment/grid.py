...
class Grid():
    ...
    def get_state(self) -> Dict[Coord, Location]:
        return self.state
    ...
    def _get_agents(self) -> Dict[Coord, Actor]:
        return {coord:location.actor for coord,location in self.state.items() if location and location.actor}
    
    def _get_dirts(self) -> Dict[Coord, DirtInterface]:
    ...
    def dirt(self, colour: Union[Colour, str]) -> DirtInterface:
        colour: Colour = Grid._as_colour(colour)
        self.dirt_count += 1

        return DirtInterface(Grid.ID_PREFIX_DIRT + str(self.dirt_count), colour)
    
    def agent(self, colour: Union[Colour, str], orientation: Union[Orientation, str]) -> Actor:
        colour: Colour = Grid._as_colour(colour)
        orientation: Orientation = Grid._as_orientation(orientation)
        self.agent_count += 1

        return Actor(Grid.ID_PREFIX_AGENT + str(self.agent_count), colour, orientation)            
    ...
    def replace_dirt(self, coordinate: Union[Coord, List[int], Tuple[int, int]], dirt) -> None:
         ...
         assert(self._in_bounds(coordinate))
         loc: Location = self.state[coordinate]
         self.state[coordinate] = Location(coordinate, loc.actor, dirt)
        
    def place_agent(self, coordinate: Union[Coord, List[int], Tuple[int, int]], agent) -> None:
        coordinate: Coord = Grid._as_coord(coordinate)
        assert(self._in_bounds(coordinate))
        assert(self.state[coordinate].actor == None)
        loc: Location = self.state[coordinate]
        self.state[coordinate] = Location(coordinate, agent, loc.dirt)
    ...
    def remove_dirt(self, coordinate: Union[Coord, List[int], Tuple[int, int]]) -> None:
        assert(self._in_bounds(coordinate))
        loc: Location = self.state[coordinate]
        self.state[coordinate] = Location(coordinate, loc.actor, None)
    ...
    def move_agent(self, _from: Union[Coord, List[int], Tuple[int, int]], _to: Union[Coord, List[int], Tuple[int, int]]) -> None:
        ...
        assert(self.state[_from].actor != None)
        assert(self.state[_to].actor == None)
        
        from_loc: Location = self.state[_from]
        to_loc: Location = self.state[_to]
        self.state[_to] = Location(to_loc.coordinate, from_loc.actor, to_loc.dirt)
    ...
    def turn_agent(self, _coordinate: Union[Coord, List[int], Tuple[int, int]], orientation: Union[Orientation, str]) -> None:
        orientation: Orientation = Grid._as_orientation(orientation)
        assert(self.state[_coordinate].actor != None)
        loc: Location = self.state[_coordinate]
        ag: Actor = loc.actor
        self.state[_coordinate] = Location(_coordinate, Actor(ag.name, ag.colour, orientation), loc.dirt)
    
    def __str__(self) -> str:
        header: str = "{0}: size: {1}, agents: {2}, dirts: {3}, ".format(str(type(self)), self.dim, self.agent_count, self.dirt_count)
        filled: Dict[Coord, Location] = {coord:location for coord,location in self.state.items()  if location is not None and (location.actor is not None or location.dirt is not None)}
